...
bopomofo_converter_instance = None

@app.on_event("startup")
def load_model():
    global cosyvoice_instance, bopomofo_converter_instance
    if cosyvoice_instance is None:
        # 可根據需求調整預設模型路徑
        model_path = "MediaTek-Research/BreezyVoice-300M"
        cosyvoice_instance = CustomCosyVoice(model_path)
    if bopomofo_converter_instance is None:
        bopomofo_converter_instance = G2PWConverter()

@app.post("/synthesize")
async def synthesize(
    speaker_prompt_audio: UploadFile = File(...),
    content_to_synthesize: str = Form(...),
    speaker_prompt_text_transcription: str = Form(None)
):
    # 儲存上傳的語音臨時檔案
    temp_audio_path = "temp_prompt.wav"
    with open(temp_audio_path, "wb") as f:
        f.write(await speaker_prompt_audio.read())
    ...
    try:
        ...
        return StreamingResponse(iterfile(), media_type="audio/wav", headers={
...
#     parser.add_argument("--speaker_prompt_text_transcription", type=str, required=False, help="Specifies the transcription of the speaker prompt audio (Highly Recommended, if not provided, the system will transcribe automatically).")
